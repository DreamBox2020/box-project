...
export class GlobalGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    this.logger.debug('GlobalGuard->canActivate->cookies', request.cookies);
    ...
    this.logger.debug('GlobalGuard->canActivate->actions', actions);
  ...
  auth(actions: string[], roles: RoleEntity[]) {
    ...
    this.logger.debug('GlobalGuard->auth->tag', tag);
