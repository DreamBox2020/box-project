...
export class PermissionService {
  ...
  can(roleIds: number[], action: string): boolean {
    this.logger.debug('Permission->can', {
      roleIds,
      action,
    });
    const documents = this.getPolicyDocumentsByRoleIds(roleIds);
    this.logger.debug('Permission->can->documents', documents);
  ...
  async getRolesByUserId(user: UserEntity | number) {
    ...
    this.logger.debug('PermissionService->getRolesByUserId', roles);
    return roles;
  }
  ...
  async getPoliciesByUserId(user: UserEntity | number) {
    ...
    this.logger.debug('PermissionService->getPoliciesByUserId', policies);
    return policies;
  }
